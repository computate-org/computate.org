...
public class WeatherObservedEnUSGenApiServiceImpl extends BaseApiServiceImpl implements WeatherObservedEnUSGenApiService {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			webClient.post(
					...
					.sendForm(MultiMap.caseInsensitiveMultiMap()
							...
							.add("permission", String.format("%s#%s", WeatherObserved.CLASS_SIMPLE_NAME, "PUT"))
			).onFailure(ex -> {
		...
		try {
			...
			if(json == null) {
				String entityShortId = siteRequest.getServiceRequest().getParams().getJsonObject("path").getString("entityShortId");
						String m = String.format("%s %s not found", "WeatherObserved", entityShortId);
					...
					if(!scopes.contains("PATCH")) {
						...
						searchWeatherObservedList(siteRequest, false, true, true).onSuccess(listWeatherObserved -> {
							try {
								...
								apiRequest.setId(Optional.ofNullable(listWeatherObserved.first()).map(o2 -> o2.getEntityShortId()).orElse(null));
	...
	public Future<WeatherObserved> patchWeatherObservedFuture(WeatherObserved o, Boolean entityShortId) {
				...
				varsWeatherObserved(siteRequest).onSuccess(a -> {
					...
					if(jsonObject.isEmpty()) {
						ngsildGetEntity(o).onSuccess(ngsildData -> {
							...
							sqlPATCHWeatherObserved(o, entityShortId).onSuccess(weatherObserved -> {
	...
	public Future<WeatherObserved> sqlPATCHWeatherObserved(WeatherObserved o, Boolean entityShortId) {
		Promise<WeatherObserved> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			for(String entityVar : methodNames) {
				switch(entityVar) {
					...
					case "setId":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(WeatherObserved.VAR_id + "=$" + num);
							num++;
					...
					case "setPressureTendency":
							o2.setPressureTendency(jsonObject.getString(entityVar));
							if(bParams.size() > 0)
								bSql.append(", ");
					...
					case "setUVIndexMax":
						...
						break;
					case "setVisibility":
							o2.setVisibility(jsonObject.getString(entityVar));
							if(bParams.size() > 0)
								bSql.append(", ");
	...
	public Future<WeatherObserved> postWeatherObservedFuture(SiteRequest siteRequest, Boolean entityShortId) {
		Promise<WeatherObserved> promise = Promise.promise();
			...
			pgPool.withTransaction(sqlConnection -> {
				...
				varsWeatherObserved(siteRequest).onSuccess(a -> {
					createWeatherObserved(siteRequest).onSuccess(weatherObserved -> {
						sqlPOSTWeatherObserved(weatherObserved, entityShortId).onSuccess(b -> {
	...
	public Future<WeatherObserved> sqlPOSTWeatherObserved(WeatherObserved o, Boolean entityShortId) {
		Promise<WeatherObserved> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
	...
	public void putimportWeatherObservedFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				apiRequest.initDeepApiRequest(siteRequest);
				siteRequest.setApiRequest_(apiRequest);
				String entityShortId = Optional.ofNullable(body.getString(WeatherObserved.VAR_entityShortId)).orElse(body.getString(WeatherObserved.VAR_solrId));
				if(Optional.ofNullable(serviceRequest.getParams()).map(p -> p.getJsonObject("query")).map( q -> q.getJsonArray("var")).orElse(new JsonArray()).stream().filter(s -> "refresh:false".equals(s)).count() > 0L) {
					siteRequest.getRequestVars().put( "refresh", "false" );
				}
					...
					try {
						if(searchList.size() >= 1) {
							...
							patchWeatherObservedFuture(o, true).onSuccess(b -> {
								LOG.debug("Import WeatherObserved {} succeeded, modified WeatherObserved. ", body.getValue(WeatherObserved.VAR_entityShortId));
								eventHandler.handle(Future.succeededFuture());
							}).onFailure(ex -> {
							...
							postWeatherObservedFuture(siteRequest, true).onSuccess(b -> {
								LOG.debug("Import WeatherObserved {} succeeded, created new WeatherObserved. ", body.getValue(WeatherObserved.VAR_entityShortId));
								eventHandler.handle(Future.succeededFuture());
							}).onFailure(ex -> {
								LOG.error(String.format("putimportWeatherObservedFuture failed. "), ex);
								eventHandler.handle(Future.failedFuture(ex));
	...
	@Override
	public void deletefilterWeatherObserved(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		LOG.debug(String.format("deletefilterWeatherObserved started. "));
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			webClient.post(
					config.getInteger(ComputateConfigKeys.AUTH_PORT)
					, config.getString(ComputateConfigKeys.AUTH_HOST_NAME)
					...
					if(!scopes.contains("DELETE")) {
						...
						searchWeatherObservedList(siteRequest, false, true, true).onSuccess(listWeatherObserved -> {
							try {
								...
								listDELETEFilterWeatherObserved(apiRequest, listWeatherObserved).onSuccess(e -> {
									response200DELETEFilterWeatherObserved(siteRequest).onSuccess(response -> {
										LOG.debug(String.format("deletefilterWeatherObserved succeeded. "));
										eventHandler.handle(Future.succeededFuture(response));
									}).onFailure(ex -> {
										LOG.error(String.format("deletefilterWeatherObserved failed. "), ex);
										error(siteRequest, eventHandler, ex);
									});
								}).onFailure(ex -> {
									LOG.error(String.format("deletefilterWeatherObserved failed. "), ex);
			...
			if("Inactive Token".equals(ex.getMessage()) || StringUtils.startsWith(ex.getMessage(), "invalid_grant:")) {
				try {
					...
					LOG.error(String.format("deletefilterWeatherObserved failed. ", ex2));
					error(null, eventHandler, ex2);
				}
			} else if(StringUtils.startsWith(ex.getMessage(), "401 UNAUTHORIZED ")) {
				eventHandler.handle(Future.succeededFuture(
	...
	public Future<Void> listDELETEFilterWeatherObserved(ApiRequest apiRequest, SearchList<WeatherObserved> listWeatherObserved) {
		Promise<Void> promise = Promise.promise();
		List<Future> futures = new ArrayList<>();
		SiteRequest siteRequest = listWeatherObserved.getSiteRequest_(SiteRequest.class);
		listWeatherObserved.getList().forEach(o -> {
			...
			o2.setSiteRequest_(siteRequest2);
			futures.add(Future.future(promise1 -> {
				deletefilterWeatherObservedFuture(o).onSuccess(a -> {
					promise1.complete();
				}).onFailure(ex -> {
					LOG.error(String.format("listDELETEFilterWeatherObserved failed. "), ex);
					promise1.fail(ex);
				});
			}));
		});
		CompositeFuture.all(futures).onSuccess( a -> {
			listWeatherObserved.next().onSuccess(next -> {
				if(next) {
					listDELETEFilterWeatherObserved(apiRequest, listWeatherObserved).onSuccess(b -> {
						promise.complete();
					}).onFailure(ex -> {
						LOG.error(String.format("listDELETEFilterWeatherObserved failed. "), ex);
						promise.fail(ex);
	...
	@Override
	public void deletefilterWeatherObservedFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				siteRequest.setJsonObject(body);
				serviceRequest.getParams().getJsonObject("query").put("rows", 1);
				searchWeatherObservedList(siteRequest, false, true, true).onSuccess(listWeatherObserved -> {
					try {
						...
						if(o != null && listWeatherObserved.getResponse().getResponse().getNumFound() == 1) {
							...
							apiRequest.setId(Optional.ofNullable(listWeatherObserved.first()).map(o2 -> o2.getEntityShortId()).orElse(null));
							apiRequest.setPk(Optional.ofNullable(listWeatherObserved.first()).map(o2 -> o2.getPk()).orElse(null));
							deletefilterWeatherObservedFuture(o).onSuccess(o2 -> {
								eventHandler.handle(Future.succeededFuture(ServiceResponse.completedWithJson(Buffer.buffer(new JsonObject().encodePrettily()))));
							}).onFailure(ex -> {
								eventHandler.handle(Future.failedFuture(ex));
							});
	...
	public Future<WeatherObserved> deletefilterWeatherObservedFuture(WeatherObserved o) {
		SiteRequest siteRequest = o.getSiteRequest_();
		Promise<WeatherObserved> promise = Promise.promise();
		...
		try {
			...
			pgPool.withTransaction(sqlConnection -> {
				siteRequest.setSqlConnection(sqlConnection);
				varsWeatherObserved(siteRequest).onSuccess(a -> {
					sqlDELETEFilterWeatherObserved(o).onSuccess(weatherObserved -> {
						relateWeatherObserved(o).onSuccess(d -> {
							unindexWeatherObserved(o).onSuccess(o2 -> {
								if(apiRequest != null) {
									apiRequest.setNumPATCH(apiRequest.getNumPATCH() + 1);
			...
			LOG.error(String.format("deletefilterWeatherObservedFuture failed. "), ex);
			promise.fail(ex);
		}
		return promise.future();
	}

	public Future<Void> sqlDELETEFilterWeatherObserved(WeatherObserved o) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			CompositeFuture.all(futures1).onSuccess(a -> {
				CompositeFuture.all(futures2).onSuccess(b -> {
					cbDeleteEntity(o).onSuccess(c -> {
						...
						LOG.error(String.format("sqlDELETEFilterWeatherObserved failed. "), ex);
						promise.fail(ex);
					});
				}).onFailure(ex -> {
					LOG.error(String.format("sqlDELETEFilterWeatherObserved failed. "), ex);
	...
	public Future<ServiceResponse> response200DELETEFilterWeatherObserved(SiteRequest siteRequest) {
		Promise<ServiceResponse> promise = Promise.promise();
		try {
			JsonObject json = new JsonObject();
			if(json == null) {
			...
			LOG.error(String.format("response200DELETEFilterWeatherObserved failed. "), ex);
			promise.fail(ex);
		}
		return promise.future();
	}
	...
	public Future<Void> cbUpsertEntity(WeatherObserved o, Boolean patch) {
		Promise<Void> promise = Promise.promise();
		try {
			...
			for (String var : vars) {
				...
				if (ngsiType != null && displayName != null && !var.equals("id") && !var.equals("ngsildData")) {
					...
					if(value != null) {
						Object ngsildVal = WeatherObserved.ngsiWeatherObserved(var, o);
						String ngsildType = WeatherObserved.ngsiType(var);
	...
	public Future<JsonObject> ngsildGetEntity(WeatherObserved o) {
	...
	public Future<Void> cbDeleteEntity(WeatherObserved o) {
		Promise<Void> promise = Promise.promise();
		try {
			webClient.delete(
					Integer.parseInt(config.getString(ComputateConfigKeys.CONTEXT_BROKER_PORT))
	...
	public Future<WeatherObserved> unindexWeatherObserved(WeatherObserved o) {
		...
		try {
			...
			o.promiseDeepForClass(siteRequest).onSuccess(a -> {
				...
				String query = String.format("filter(entityShortId_docvalues_string:%s)", o.obtainForClass("entityShortId"));
	...
	public Future<JsonObject> generatePageBody(ComputateSiteRequest siteRequest, Map<String, Object> ctx, String resourceUri, String templateUri, String classSimpleName) {
		...
		try {
			...
			page.persistForClass(WeatherObserved.VAR_id, WeatherObserved.staticSetId(siteRequest2, (String)result.get(WeatherObserved.VAR_id)));
