...
public class WeatherObservedGenPage extends WeatherObservedGenPageGen<PageLayout> {
   ...
   * Ignore: true
   **/
  protected void _searchListWeatherObserved_(Wrap<SearchList<WeatherObserved>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListWeatherObserved_ != null)
      w.o(JsonObject.mapFrom(searchListWeatherObserved_.getResponse()).toString());
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(searchListWeatherObserved_.getResponse().getStats());
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(searchListWeatherObserved_.getResponse().getFacetCounts());
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListWeatherObserved_.getStart().longValue();
    Long rows = searchListWeatherObserved_.getRows().longValue();
    Long foundNum = searchListWeatherObserved_.getResponse().getResponse().getNumFound().longValue();
  ...
  protected void _varsQ(JsonObject vars) {
    WeatherObserved.varsQForClass().forEach(var -> {
      ...
      json.put("val", Optional.ofNullable(searchListWeatherObserved_.getRequest().getQuery()).filter(fq -> fq.startsWith(WeatherObserved.varIndexedWeatherObserved(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsRange(JsonObject vars) {
    WeatherObserved.varsRangeForClass().forEach(var -> {
      String varIndexed = WeatherObserved.varIndexedWeatherObserved(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(WeatherObserved.displayNameWeatherObserved(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = searchListWeatherObserved_.getResponse().getResponse().getNumFound().longValue();
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListWeatherObserved_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListWeatherObserved_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListWeatherObserved_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = WeatherObserved.searchVarWeatherObserved(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", WeatherObserved.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListWeatherObserved_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = WeatherObserved.searchVarWeatherObserved(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", WeatherObserved.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return WeatherObserved.searchVarWeatherObserved(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListWeatherObserved_.getFacetPivotMinCount()).orElse(0));
  }
    ...
    Optional.ofNullable(searchListWeatherObserved_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = WeatherObserved.searchVarWeatherObserved(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListWeatherObserved_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListWeatherObserved_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listWeatherObserved(JsonArray l) {
