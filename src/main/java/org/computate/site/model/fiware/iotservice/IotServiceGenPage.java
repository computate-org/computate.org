...
public class IotServiceGenPage extends IotServiceGenPageGen<PageLayout> {
   ...
   * Ignore: true
   **/
  protected void _searchListIotService_(Wrap<SearchList<IotService>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListIotService_ != null)
      w.o(JsonObject.mapFrom(searchListIotService_.getResponse()).toString());
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(searchListIotService_.getResponse().getStats());
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(searchListIotService_.getResponse().getFacetCounts());
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListIotService_.getStart().longValue();
    Long rows = searchListIotService_.getRows().longValue();
    Long foundNum = searchListIotService_.getResponse().getResponse().getNumFound().longValue();
  ...
  protected void _varsQ(JsonObject vars) {
    IotService.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(IotService.displayNameIotService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(IotService.classSimpleNameIotService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListIotService_.getRequest().getQuery()).filter(fq -> fq.startsWith(IotService.varIndexedIotService(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    for(String var : IotService.varsFqForClass()) {
      String varIndexed = IotService.varIndexedIotService(var);
      String varStored = IotService.varStoredIotService(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(IotService.displayNameIotService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(IotService.classSimpleNameIotService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListIotService_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(IotService.varIndexedIotService(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        json.put("val", v);
        varsFqCount++;
      }
  ...
  protected void _varsRange(JsonObject vars) {
    IotService.varsRangeForClass().forEach(var -> {
      String varIndexed = IotService.varIndexedIotService(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(IotService.displayNameIotService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(IotService.classSimpleNameIotService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListIotService_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(IotService.varIndexedIotService(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = searchListIotService_.getResponse().getResponse().getNumFound().longValue();
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListIotService_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListIotService_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListIotService_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = IotService.searchVarIotService(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", IotService.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListIotService_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = IotService.searchVarIotService(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", IotService.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListIotService_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListIotService_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListIotService_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListIotService_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return IotService.searchVarIotService(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListIotService_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListIotService_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListIotService_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListIotService_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListIotService_.getDefaultSort()) {
    ...
    Optional.ofNullable(searchListIotService_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = IotService.searchVarIotService(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListIotService_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListIotService_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listIotService(JsonArray l) {
  ...
  **/
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("A service within a service path in the NGSI-LD standard");
  }
  ...
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"fa-solid fa-tower-cell\"></i>");
  }
