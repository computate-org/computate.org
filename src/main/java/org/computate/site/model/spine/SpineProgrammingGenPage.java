 ...
 * Generated: true
 **/
public class SpineProgrammingGenPage extends SpineProgrammingGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListSpineProgramming_(Wrap<SearchList<SpineProgramming>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListSpineProgramming_ != null)
      w.o(Optional.ofNullable(searchListSpineProgramming_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListSpineProgramming_.getStart().longValue();
    Long rows = searchListSpineProgramming_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListSpineProgramming_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListSpineProgramming_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    SpineProgramming.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(SpineProgramming.displayNameSpineProgramming(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SpineProgramming.classSimpleNameSpineProgramming(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListSpineProgramming_.getRequest().getQuery()).filter(fq -> fq.startsWith(SpineProgramming.varIndexedSpineProgramming(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : SpineProgramming.varsFqForClass()) {
      String varIndexed = SpineProgramming.varIndexedSpineProgramming(var);
      String varStored = SpineProgramming.varStoredSpineProgramming(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(SpineProgramming.displayNameSpineProgramming(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SpineProgramming.classSimpleNameSpineProgramming(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListSpineProgramming_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SpineProgramming.varIndexedSpineProgramming(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = SpineProgramming.searchVarSpineProgramming(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    SpineProgramming.varsRangeForClass().forEach(var -> {
      String varIndexed = SpineProgramming.varIndexedSpineProgramming(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(SpineProgramming.displayNameSpineProgramming(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SpineProgramming.classSimpleNameSpineProgramming(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListSpineProgramming_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SpineProgramming.varIndexedSpineProgramming(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListSpineProgramming_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListSpineProgramming_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListSpineProgramming_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListSpineProgramming_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListSpineProgramming_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = SpineProgramming.searchVarSpineProgramming(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", SpineProgramming.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListSpineProgramming_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = SpineProgramming.searchVarSpineProgramming(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", SpineProgramming.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return SpineProgramming.searchVarSpineProgramming(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSpineProgramming_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListSpineProgramming_.getDefaultSort()) {
      Optional.ofNullable(searchListSpineProgramming_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = SpineProgramming.searchVarSpineProgramming(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListSpineProgramming_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = SpineProgramming.searchVarSpineProgramming(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListSpineProgramming_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListSpineProgramming_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listSpineProgramming(JsonArray l) {
    Optional.ofNullable(searchListSpineProgramming_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListSpineProgramming_ == null ? 0 : searchListSpineProgramming_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<SpineProgramming> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("pageId")).orElse(null) != null)
      w.o(searchListSpineProgramming_.get(0));
  }
  ...
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/spine-programming");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Learn how to become a SPINE Programmer building data-driven websites and increase your developer productivity up to 4900%. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/spine-programming-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"fa-kit fa-spine\"></i>");
  }

  protected void _pageUriSpineProgramming(Wrap<String> c) {
      c.o("/en-us/search/spine-programming");
  }
}
