 ...
 * Generated: true
 **/
public class DeploySpineGenPage extends DeploySpineGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListDeploySpine_(Wrap<SearchList<DeploySpine>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListDeploySpine_ != null)
      w.o(Optional.ofNullable(searchListDeploySpine_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListDeploySpine_.getStart().longValue();
    Long rows = searchListDeploySpine_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListDeploySpine_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListDeploySpine_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    DeploySpine.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(DeploySpine.displayNameDeploySpine(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(DeploySpine.classSimpleNameDeploySpine(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListDeploySpine_.getRequest().getQuery()).filter(fq -> fq.startsWith(DeploySpine.varIndexedDeploySpine(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : DeploySpine.varsFqForClass()) {
      String varIndexed = DeploySpine.varIndexedDeploySpine(var);
      String varStored = DeploySpine.varStoredDeploySpine(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(DeploySpine.displayNameDeploySpine(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(DeploySpine.classSimpleNameDeploySpine(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListDeploySpine_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(DeploySpine.varIndexedDeploySpine(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = DeploySpine.searchVarDeploySpine(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    DeploySpine.varsRangeForClass().forEach(var -> {
      String varIndexed = DeploySpine.varIndexedDeploySpine(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(DeploySpine.displayNameDeploySpine(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(DeploySpine.classSimpleNameDeploySpine(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListDeploySpine_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(DeploySpine.varIndexedDeploySpine(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListDeploySpine_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListDeploySpine_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListDeploySpine_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListDeploySpine_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListDeploySpine_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = DeploySpine.searchVarDeploySpine(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", DeploySpine.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListDeploySpine_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = DeploySpine.searchVarDeploySpine(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", DeploySpine.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListDeploySpine_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListDeploySpine_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListDeploySpine_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return DeploySpine.searchVarDeploySpine(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListDeploySpine_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListDeploySpine_.getDefaultSort()) {
      Optional.ofNullable(searchListDeploySpine_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = DeploySpine.searchVarDeploySpine(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListDeploySpine_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = DeploySpine.searchVarDeploySpine(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListDeploySpine_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListDeploySpine_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listDeploySpine(JsonArray l) {
    Optional.ofNullable(searchListDeploySpine_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListDeploySpine_ == null ? 0 : searchListDeploySpine_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<DeploySpine> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("pageId")).orElse(null) != null)
      w.o(searchListDeploySpine_.get(0));
  }
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/deploy-spine");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/deploy-spine");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Learn how to deploy SPINE apps to OpenShift Local to run community driven open source cloud applications built with SPINE Programming Theory in OpenShift Local on your own computer. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/deploy-spine-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"fa-kit fa-openshift\"></i>");
  }

  protected void _pageUriDeploySpine(Wrap<String> c) {
      c.o("/en-us/search/deploy-spine");
  }
}
