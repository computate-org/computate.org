 ...
 * Generated: true
 **/
public class CompanyResearchGenPage extends CompanyResearchGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListCompanyResearch_(Wrap<SearchList<CompanyResearch>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListCompanyResearch_ != null)
      w.o(Optional.ofNullable(searchListCompanyResearch_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListCompanyResearch_.getStart().longValue();
    Long rows = searchListCompanyResearch_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListCompanyResearch_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyResearch_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    CompanyResearch.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(CompanyResearch.displayNameCompanyResearch(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CompanyResearch.classSimpleNameCompanyResearch(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListCompanyResearch_.getRequest().getQuery()).filter(fq -> fq.startsWith(CompanyResearch.varIndexedCompanyResearch(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : CompanyResearch.varsFqForClass()) {
      String varIndexed = CompanyResearch.varIndexedCompanyResearch(var);
      String varStored = CompanyResearch.varStoredCompanyResearch(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(CompanyResearch.displayNameCompanyResearch(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CompanyResearch.classSimpleNameCompanyResearch(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListCompanyResearch_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(CompanyResearch.varIndexedCompanyResearch(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = CompanyResearch.searchVarCompanyResearch(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    CompanyResearch.varsRangeForClass().forEach(var -> {
      String varIndexed = CompanyResearch.varIndexedCompanyResearch(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(CompanyResearch.displayNameCompanyResearch(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CompanyResearch.classSimpleNameCompanyResearch(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListCompanyResearch_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(CompanyResearch.varIndexedCompanyResearch(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListCompanyResearch_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyResearch_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListCompanyResearch_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListCompanyResearch_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListCompanyResearch_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = CompanyResearch.searchVarCompanyResearch(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", CompanyResearch.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListCompanyResearch_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = CompanyResearch.searchVarCompanyResearch(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", CompanyResearch.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return CompanyResearch.searchVarCompanyResearch(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCompanyResearch_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListCompanyResearch_.getDefaultSort()) {
      Optional.ofNullable(searchListCompanyResearch_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = CompanyResearch.searchVarCompanyResearch(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListCompanyResearch_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = CompanyResearch.searchVarCompanyResearch(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListCompanyResearch_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListCompanyResearch_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listCompanyResearch(JsonArray l) {
    Optional.ofNullable(searchListCompanyResearch_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListCompanyResearch_ == null ? 0 : searchListCompanyResearch_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<CompanyResearch> w) {
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("research");
    else if(searchListCompanyResearch_ == null || resultCount == 0)
      c.o("no research found");
    else
      c.o("research");
  }
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/research");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/research");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Read about the computate project related research going on at Red Hat research");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/research-999.png");
  }
