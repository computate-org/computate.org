 ...
 * Generated: true
 **/
public class ComputateEventGenPage extends ComputateEventGenPageGen<BaseModelPage> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListComputateEvent_(Wrap<SearchList<ComputateEvent>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListComputateEvent_ != null)
      w.o(JsonObject.mapFrom(searchListComputateEvent_.getResponse()).toString());
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(searchListComputateEvent_.getResponse().getStats());
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(searchListComputateEvent_.getResponse().getFacetCounts());
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListComputateEvent_.getStart().longValue();
    Long rows = searchListComputateEvent_.getRows().longValue();
    Long foundNum = searchListComputateEvent_.getResponse().getResponse().getNumFound().longValue();
  ...
  protected void _varsQ(JsonObject vars) {
    ComputateEvent.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ComputateEvent.displayNameComputateEvent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ComputateEvent.classSimpleNameComputateEvent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListComputateEvent_.getRequest().getQuery()).filter(fq -> fq.startsWith(ComputateEvent.varIndexedComputateEvent(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    ComputateEvent.varsFqForClass().forEach(var -> {
      String varIndexed = ComputateEvent.varIndexedComputateEvent(var);
      String varStored = ComputateEvent.varStoredComputateEvent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(ComputateEvent.displayNameComputateEvent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ComputateEvent.classSimpleNameComputateEvent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListComputateEvent_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ComputateEvent.varIndexedComputateEvent(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
  ...
  protected void _varsRange(JsonObject vars) {
    ComputateEvent.varsRangeForClass().forEach(var -> {
      String varIndexed = ComputateEvent.varIndexedComputateEvent(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ComputateEvent.displayNameComputateEvent(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = searchListComputateEvent_.getResponse().getResponse().getNumFound().longValue();
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListComputateEvent_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListComputateEvent_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListComputateEvent_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = ComputateEvent.searchVarComputateEvent(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "deleted_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", ComputateEvent.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListComputateEvent_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = ComputateEvent.searchVarComputateEvent(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", ComputateEvent.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListComputateEvent_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListComputateEvent_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListComputateEvent_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListComputateEvent_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return ComputateEvent.searchVarComputateEvent(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListComputateEvent_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListComputateEvent_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListComputateEvent_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListComputateEvent_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListComputateEvent_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = ComputateEvent.searchVarComputateEvent(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListComputateEvent_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListComputateEvent_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listComputateEvent(JsonArray l) {
    Optional.ofNullable(searchListComputateEvent_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _computateEventCount(Wrap<Integer> w) {
    w.o(searchListComputateEvent_ == null ? 0 : searchListComputateEvent_.size());
  }

  protected void _computateEvent_(Wrap<ComputateEvent> w) {
    if(computateEventCount == 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
      w.o(searchListComputateEvent_.get(0));
  }

  protected void _pk(Wrap<Long> w) {
    if(computateEvent_ != null)
      w.o(computateEvent_.getPk());
  }

  protected void _id(Wrap<String> w) {
    if(computateEvent_ != null)
      w.o(computateEvent_.getId());
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(computateEvent_ != null && computateEvent_.getObjectTitle() != null)
      c.o(computateEvent_.getObjectTitle());
    else if(computateEvent_ != null)
      c.o("events");
    else if(searchListComputateEvent_ == null || computateEventCount == 0)
      c.o("no event found");
    else
      c.o("events");
  }
