 ...
 * Generated: true
 **/
public class CompanyWebsiteGenPage extends CompanyWebsiteGenPageGen<PageLayout> {
  ...
  protected void _pageResponse(Wrap<String> w) {
    if(searchListCompanyWebsite_ != null)
      w.o(Optional.ofNullable(searchListCompanyWebsite_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListCompanyWebsite_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListCompanyWebsite_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }
  ...
  protected void _pagination(JsonObject pagination) {
    ...
    Long foundNum = Optional.ofNullable(searchListCompanyWebsite_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyWebsite_.getList().size()));
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : CompanyWebsite.varsFqForClass()) {
      ...
      json.put("displayName", Optional.ofNullable(CompanyWebsite.displayNameCompanyWebsite(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CompanyWebsite.classSimpleNameCompanyWebsite(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListCompanyWebsite_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(CompanyWebsite.varIndexedCompanyWebsite(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = CompanyWebsite.searchVarCompanyWebsite(varIndexed);
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListCompanyWebsite_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyWebsite_.getList().size()));
    String q = "*:*";
   ...
   * Initialized: false
  **/
  protected void _result(Wrap<CompanyWebsite> w) {
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("websites");
    else if(searchListCompanyWebsite_ == null || resultCount == 0)
  ...
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/website-999.png");
  }
