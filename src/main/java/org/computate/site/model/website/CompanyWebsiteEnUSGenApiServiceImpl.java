...
public class CompanyWebsiteEnUSGenApiServiceImpl extends BaseApiServiceImpl implements CompanyWebsiteEnUSGenApiService {
	...
	public void patchCompanyWebsite(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			form.add("permission", String.format("%s#%s", CompanyWebsite.CLASS_AUTH_RESOURCE, config.getString(ComputateConfigKeys.AUTH_SCOPE_ADMIN)));
			form.add("permission", String.format("%s#%s", CompanyWebsite.CLASS_AUTH_RESOURCE, config.getString(ComputateConfigKeys.AUTH_SCOPE_SUPER_ADMIN)));
			...
			form.add("permission", String.format("%s#%s", CompanyWebsite.CLASS_AUTH_RESOURCE, "PUT"));
			if(pageId != null)
	...
	public void searchpageCompanyWebsitePageInit(CompanyWebsitePage page, SearchList<CompanyWebsite> listCompanyWebsite, Promise<Void> promise) {
		promise.complete();
	}
		...
		try {
			...
			page.promiseDeepCompanyWebsitePage(siteRequest).onSuccess(a -> {
				try {
					...
					searchpageCompanyWebsitePageInit(page, listCompanyWebsite, promise1);
	...
	public void editpageCompanyWebsitePageInit(CompanyWebsitePage page, SearchList<CompanyWebsite> listCompanyWebsite, Promise<Void> promise) {
		promise.complete();
	}
		...
		try {
			...
			page.promiseDeepCompanyWebsitePage(siteRequest).onSuccess(a -> {
				try {
					...
					ctx.mergeIn(JsonObject.mapFrom(page));
					Promise<Void> promise1 = Promise.promise();
					editpageCompanyWebsitePageInit(page, listCompanyWebsite, promise1);
					promise1.future().onSuccess(b -> {
						String renderedTemplate = jinjava.render(template, ctx.getMap());
						Buffer buffer = Buffer.buffer(renderedTemplate);
						promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
	...
	public void displaypageCompanyWebsitePageInit(CompanyWebsitePage page, SearchList<CompanyWebsite> listCompanyWebsite, Promise<Void> promise) {
		promise.complete();
	}
		...
		try {
			...
			page.promiseDeepCompanyWebsitePage(siteRequest).onSuccess(a -> {
				try {
					...
					ctx.mergeIn(JsonObject.mapFrom(page));
					Promise<Void> promise1 = Promise.promise();
					displaypageCompanyWebsitePageInit(page, listCompanyWebsite, promise1);
					promise1.future().onSuccess(b -> {
						String renderedTemplate = jinjava.render(template, ctx.getMap());
						Buffer buffer = Buffer.buffer(renderedTemplate);
						promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
