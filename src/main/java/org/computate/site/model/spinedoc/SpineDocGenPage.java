 ...
 * Generated: true
 **/
public class SpineDocGenPage extends SpineDocGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListSpineDoc_(Wrap<SearchList<SpineDoc>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListSpineDoc_ != null)
      w.o(Optional.ofNullable(searchListSpineDoc_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListSpineDoc_.getStart().longValue();
    Long rows = searchListSpineDoc_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListSpineDoc_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListSpineDoc_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    SpineDoc.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(SpineDoc.displayNameSpineDoc(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SpineDoc.classSimpleNameSpineDoc(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListSpineDoc_.getRequest().getQuery()).filter(fq -> fq.startsWith(SpineDoc.varIndexedSpineDoc(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : SpineDoc.varsFqForClass()) {
      String varIndexed = SpineDoc.varIndexedSpineDoc(var);
      String varStored = SpineDoc.varStoredSpineDoc(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(SpineDoc.displayNameSpineDoc(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SpineDoc.classSimpleNameSpineDoc(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListSpineDoc_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SpineDoc.varIndexedSpineDoc(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = SpineDoc.searchVarSpineDoc(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    SpineDoc.varsRangeForClass().forEach(var -> {
      String varIndexed = SpineDoc.varIndexedSpineDoc(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(SpineDoc.displayNameSpineDoc(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SpineDoc.classSimpleNameSpineDoc(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListSpineDoc_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SpineDoc.varIndexedSpineDoc(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListSpineDoc_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListSpineDoc_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListSpineDoc_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListSpineDoc_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListSpineDoc_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = SpineDoc.searchVarSpineDoc(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", SpineDoc.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListSpineDoc_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = SpineDoc.searchVarSpineDoc(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", SpineDoc.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListSpineDoc_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListSpineDoc_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListSpineDoc_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return SpineDoc.searchVarSpineDoc(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSpineDoc_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListSpineDoc_.getDefaultSort()) {
      Optional.ofNullable(searchListSpineDoc_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = SpineDoc.searchVarSpineDoc(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListSpineDoc_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = SpineDoc.searchVarSpineDoc(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListSpineDoc_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListSpineDoc_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listSpineDoc(JsonArray l) {
    Optional.ofNullable(searchListSpineDoc_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListSpineDoc_ == null ? 0 : searchListSpineDoc_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<SpineDoc> w) {
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("SPINE docs");
    else if(searchListSpineDoc_ == null || resultCount == 0)
      c.o("no SPINE doc found");
    else
      c.o("SPINE docs");
  }
  ...
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/spine-doc");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/spine-doc");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Learn about the bones that form the platform and data models built with SPINE Programming Theory and increase your developer productivity up to 4900%. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/spine-doc-999.png");
  }
  ...
  protected void _pageUriSpineDoc(Wrap<String> c) {
      c.o("/en-us/search/spine-doc");
  }
}
