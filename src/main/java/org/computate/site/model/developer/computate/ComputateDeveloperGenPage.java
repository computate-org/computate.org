 ...
 * Generated: true
 **/
public class ComputateDeveloperGenPage extends ComputateDeveloperGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListComputateDeveloper_(Wrap<SearchList<ComputateDeveloper>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListComputateDeveloper_ != null)
      w.o(Optional.ofNullable(searchListComputateDeveloper_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListComputateDeveloper_.getStart().longValue();
    Long rows = searchListComputateDeveloper_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListComputateDeveloper_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListComputateDeveloper_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    ComputateDeveloper.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ComputateDeveloper.displayNameComputateDeveloper(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ComputateDeveloper.classSimpleNameComputateDeveloper(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListComputateDeveloper_.getRequest().getQuery()).filter(fq -> fq.startsWith(ComputateDeveloper.varIndexedComputateDeveloper(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : ComputateDeveloper.varsFqForClass()) {
      String varIndexed = ComputateDeveloper.varIndexedComputateDeveloper(var);
      String varStored = ComputateDeveloper.varStoredComputateDeveloper(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(ComputateDeveloper.displayNameComputateDeveloper(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ComputateDeveloper.classSimpleNameComputateDeveloper(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListComputateDeveloper_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ComputateDeveloper.varIndexedComputateDeveloper(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = ComputateDeveloper.searchVarComputateDeveloper(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    ComputateDeveloper.varsRangeForClass().forEach(var -> {
      String varIndexed = ComputateDeveloper.varIndexedComputateDeveloper(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(ComputateDeveloper.displayNameComputateDeveloper(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(ComputateDeveloper.classSimpleNameComputateDeveloper(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListComputateDeveloper_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(ComputateDeveloper.varIndexedComputateDeveloper(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListComputateDeveloper_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListComputateDeveloper_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListComputateDeveloper_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListComputateDeveloper_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListComputateDeveloper_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = ComputateDeveloper.searchVarComputateDeveloper(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", ComputateDeveloper.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListComputateDeveloper_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = ComputateDeveloper.searchVarComputateDeveloper(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", ComputateDeveloper.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return ComputateDeveloper.searchVarComputateDeveloper(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListComputateDeveloper_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListComputateDeveloper_.getDefaultSort()) {
      Optional.ofNullable(searchListComputateDeveloper_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = ComputateDeveloper.searchVarComputateDeveloper(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListComputateDeveloper_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = ComputateDeveloper.searchVarComputateDeveloper(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListComputateDeveloper_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListComputateDeveloper_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listComputateDeveloper(JsonArray l) {
    Optional.ofNullable(searchListComputateDeveloper_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListComputateDeveloper_ == null ? 0 : searchListComputateDeveloper_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<ComputateDeveloper> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("pageId")).orElse(null) != null)
      w.o(searchListComputateDeveloper_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("Computate Developers");
    else if(searchListComputateDeveloper_ == null || resultCount == 0)
      c.o("no Computate Developer found");
    else
      c.o("Computate Developers");
  }
  ...
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/computate-developer");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Learn how to become a Computate platform developer — Learn how to build your own data-driven website on your own computer for your own ideas. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/computate-developer-999.png");
  }
  ...
  protected void _pageUriComputateDeveloper(Wrap<String> c) {
      c.o("/en-us/search/computate-developer");
  }
}
