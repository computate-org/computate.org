...
public class CompanyServiceGenPage extends CompanyServiceGenPageGen<PageLayout> {
  ...
  protected void _pageResponse(Wrap<String> w) {
    if(searchListCompanyService_ != null)
      w.o(Optional.ofNullable(searchListCompanyService_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListCompanyService_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListCompanyService_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }
  ...
  protected void _pagination(JsonObject pagination) {
    ...
    Long foundNum = Optional.ofNullable(searchListCompanyService_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyService_.getList().size()));
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : CompanyService.varsFqForClass()) {
      ...
      json.put("displayName", Optional.ofNullable(CompanyService.displayNameCompanyService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CompanyService.classSimpleNameCompanyService(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListCompanyService_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(CompanyService.varIndexedCompanyService(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = CompanyService.searchVarCompanyService(varIndexed);
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListCompanyService_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListCompanyService_.getList().size()));
    String q = "*:*";
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListCompanyService_.getDefaultSort()) {
