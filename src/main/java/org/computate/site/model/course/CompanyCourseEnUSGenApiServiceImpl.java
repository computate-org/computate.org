...
public class CompanyCourseEnUSGenApiServiceImpl extends BaseApiServiceImpl implements CompanyCourseEnUSGenApiService {
	...
	public CompanyCourseEnUSGenApiServiceImpl(EventBus eventBus, JsonObject config, WorkerExecutor workerExecutor, PgPool pgPool, KafkaProducer<String, String> kafkaProducer, WebClient webClient, OAuth2Auth oauth2AuthenticationProvider, AuthorizationProvider authorizationProvider, Jinjava jinjava) {
				...
				try {
					...
					if(!scopes.contains("PATCH")) {
						...
						searchCompanyCourseList(siteRequest, false, true, true).onSuccess(listCompanyCourse -> {
							try {
								if(listCompanyCourse.getResponse().getResponse().getNumFound() > 1
										&& !Optional.ofNullable(config.getString(ComputateConfigKeys.AUTH_ROLE_REQUIRED + "_CompanyCourse")).map(v -> RoleBasedAuthorization.create(v).match(siteRequest.getUser())).orElse(false)
										) {
									String message = String.format("roles required: " + config.getString(ComputateConfigKeys.AUTH_ROLE_REQUIRED + "_CompanyCourse"));
									LOG.error(message);
									...
									apiRequest.setPk(Optional.ofNullable(listCompanyCourse.first()).map(o2 -> o2.getPk()).orElse(null));
	...
	public Future<CompanyCourse> patchCompanyCourseFuture(CompanyCourse o, Boolean inheritPk) {
		...
		try {
			...
			Promise<CompanyCourse> promise1 = Promise.promise();
			pgPool.withTransaction(sqlConnection -> {
				siteRequest.setSqlConnection(sqlConnection);
				varsCompanyCourse(siteRequest).onSuccess(a -> {
					sqlPATCHCompanyCourse(o, inheritPk).onSuccess(companyCourse -> {
						persistCompanyCourse(companyCourse).onSuccess(c -> {
							relateCompanyCourse(companyCourse).onSuccess(d -> {
								indexCompanyCourse(companyCourse).onSuccess(o2 -> {
									if(apiRequest != null) {
										apiRequest.setNumPATCH(apiRequest.getNumPATCH() + 1);
										if(apiRequest.getNumFound() == 1L && Optional.ofNullable(siteRequest.getJsonObject()).map(json -> json.size() > 0).orElse(false)) {
											o2.apiRequestCompanyCourse();
											if(apiRequest.getVars().size() > 0)
												eventBus.publish("websocketCompanyCourse", JsonObject.mapFrom(apiRequest).toString());
										}
									}
									promise1.complete(companyCourse);
								}).onFailure(ex -> {
									promise1.fail(ex);
								});
							}).onFailure(ex -> {
				...
				siteRequest.setSqlConnection(null);
				promise.fail(ex);
			}).compose(companyCourse -> {
				Promise<CompanyCourse> promise2 = Promise.promise();
				refreshCompanyCourse(companyCourse).onSuccess(a -> {
					promise2.complete(companyCourse);
				}).onFailure(ex -> {
					promise2.fail(ex);
				});
				return promise2.future();
			}).onSuccess(companyCourse -> {
	...
	public Future<CompanyCourse> sqlPATCHCompanyCourse(CompanyCourse o, Boolean inheritPk) {
		Promise<CompanyCourse> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			Integer num = 1;
			StringBuilder bSql = new StringBuilder("UPDATE CompanyCourse SET ");
			...
			for(String entityVar : methodNames) {
				switch(entityVar) {
					case "setInheritPk":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_inheritPk + "=$" + num);
							num++;
					...
					case "setCreated":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_created + "=$" + num);
							num++;
					...
					case "setArchived":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_archived + "=$" + num);
							num++;
					...
					case "setDeleted":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_deleted + "=$" + num);
							num++;
					...
					case "setSessionId":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_sessionId + "=$" + num);
							num++;
					...
					case "setUserKey":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_userKey + "=$" + num);
							num++;
					...
					case "setCourseName":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_courseName + "=$" + num);
							num++;
					...
					case "setCourseDescription":
							...
							if(bParams.size() > 0)
								bSql.append(", ");
							bSql.append(CompanyCourse.VAR_courseDescription + "=$" + num);
							num++;
			...
			if(bParams.size() > 0) {
				...
				futures2.add(0, Future.future(a -> {
					sqlConnection.preparedQuery(bSql.toString())
						...
						RuntimeException ex2 = new RuntimeException("value CompanyCourse failed", ex);
						LOG.error(String.format("relateCompanyCourse failed. "), ex2);
			...
			CompositeFuture.all(futures1).onSuccess(a -> {
				CompositeFuture.all(futures2).onSuccess(b -> {
					CompanyCourse o3 = new CompanyCourse();
					o3.setSiteRequest_(o.getSiteRequest_());
					o3.setPk(pk);
					promise.complete(o3);
				}).onFailure(ex -> {
					LOG.error(String.format("sqlPATCHCompanyCourse failed. "), ex);
					promise.fail(ex);
				});
			}).onFailure(ex -> {
				LOG.error(String.format("sqlPATCHCompanyCourse failed. "), ex);
			...
			pgPool.withTransaction(sqlConnection -> {
				...
				varsCompanyCourse(siteRequest).onSuccess(a -> {
					createCompanyCourse(siteRequest).onSuccess(companyCourse -> {
						sqlPOSTCompanyCourse(companyCourse, inheritPk).onSuccess(b -> {
							persistCompanyCourse(companyCourse).onSuccess(c -> {
								relateCompanyCourse(companyCourse).onSuccess(d -> {
									indexCompanyCourse(companyCourse).onSuccess(o2 -> {
										promise1.complete(companyCourse);
				...
				refreshCompanyCourse(companyCourse).onSuccess(a -> {
					try {
						...
						if(apiRequest != null) {
							apiRequest.setNumPATCH(apiRequest.getNumPATCH() + 1);
							companyCourse.apiRequestCompanyCourse();
							eventBus.publish("websocketCompanyCourse", JsonObject.mapFrom(apiRequest).toString());
						}
						promise2.complete(companyCourse);
					} catch(Exception ex) {
	...
	public Future<Void> sqlPOSTCompanyCourse(CompanyCourse o, Boolean inheritPk) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			if(jsonObject != null) {
				...
				for(String entityVar : entityVars) {
					switch(entityVar) {
					case CompanyCourse.VAR_inheritPk:
						o2.setInheritPk(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlInheritPk());
						break;
					case CompanyCourse.VAR_created:
						o2.setCreated(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlCreated());
						break;
					case CompanyCourse.VAR_archived:
						o2.setArchived(jsonObject.getBoolean(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlArchived());
						break;
					case CompanyCourse.VAR_deleted:
						o2.setDeleted(jsonObject.getBoolean(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlDeleted());
						break;
					case CompanyCourse.VAR_sessionId:
						o2.setSessionId(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlSessionId());
						break;
					case CompanyCourse.VAR_userKey:
						o2.setUserKey(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlUserKey());
						break;
					case CompanyCourse.VAR_courseName:
						o2.setCourseName(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
						...
						bParams.add(o2.sqlCourseName());
						break;
					case CompanyCourse.VAR_courseDescription:
						o2.setCourseDescription(jsonObject.getString(entityVar));
						if(bParams.size() > 0) {
							bSql.append(", ");
						}
			...
			CompositeFuture.all(futures1).onSuccess(a -> {
				CompositeFuture.all(futures2).onSuccess(b -> {
					...
					LOG.error(String.format("sqlPOSTCompanyCourse failed. "), ex);
					promise.fail(ex);
				});
			}).onFailure(ex -> {
				LOG.error(String.format("sqlPOSTCompanyCourse failed. "), ex);
	...
	public void putimportCompanyCourseFuture(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture").onSuccess(siteRequest -> {
			try {
				...
				apiRequest.initDeepApiRequest(siteRequest);
				siteRequest.setApiRequest_(apiRequest);
				String inheritPk = Optional.ofNullable(body.getString(CompanyCourse.VAR_pk)).orElse(body.getString(CompanyCourse.VAR_id));
					...
					try {
						if(searchList.size() >= 1) {
							...
							if(body2.size() > 0) {
								...
								patchCompanyCourseFuture(o, true).onSuccess(b -> {
									LOG.debug("Import CompanyCourse {} succeeded, modified CompanyCourse. ", body.getValue(CompanyCourse.VAR_pk));
									eventHandler.handle(Future.succeededFuture());
								}).onFailure(ex -> {
							...
							postCompanyCourseFuture(siteRequest, true).onSuccess(b -> {
								LOG.debug("Import CompanyCourse {} succeeded, created new CompanyCourse. ", body.getValue(CompanyCourse.VAR_pk));
								eventHandler.handle(Future.succeededFuture());
							}).onFailure(ex -> {
								LOG.error(String.format("putimportCompanyCourseFuture failed. "), ex);
								eventHandler.handle(Future.failedFuture(ex));
	...
	public Future<CompanyCourse> createCompanyCourse(SiteRequest siteRequest) {
		...
		try {
			...
			sqlConnection.preparedQuery("INSERT INTO CompanyCourse(created, userKey) VALUES($1, $2) RETURNING pk")
				...
				LOG.error("createCompanyCourse failed. ", ex2);
				promise.fail(ex2);
			});
		} catch(Exception ex) {
	...
	public Future<Void> persistCompanyCourse(CompanyCourse o) {
		Promise<Void> promise = Promise.promise();
		try {
			SiteRequest siteRequest = o.getSiteRequest_();
			SqlConnection sqlConnection = siteRequest.getSqlConnection();
			Long pk = o.getPk();
			sqlConnection.preparedQuery("SELECT * FROM CompanyCourse WHERE pk=$1")
					.collecting(Collectors.toList())
					.execute(Tuple.of(pk)
					).onSuccess(result -> {
				try {
				...
				RuntimeException ex2 = new RuntimeException(ex);
				LOG.error(String.format("persistCompanyCourse failed. "), ex2);
				promise.fail(ex2);
			});
		} catch(Exception ex) {
			LOG.error(String.format("persistCompanyCourse failed. "), ex);
	...
	public Future<Void> relateCompanyCourse(CompanyCourse o) {
		Promise<Void> promise = Promise.promise();
			promise.complete();
		return promise.future();
	}
	...
	public Future<Void> refreshCompanyCourse(CompanyCourse o) {
		Promise<Void> promise = Promise.promise();
		SiteRequest siteRequest = o.getSiteRequest_();
		try {
			ApiRequest apiRequest = siteRequest.getApiRequest_();
			...
			if(refresh && !Optional.ofNullable(siteRequest.getJsonObject()).map(JsonObject::isEmpty).orElse(true)) {
				...
				CompositeFuture.all(futures).onSuccess(b -> {
					...
					JsonObject context = new JsonObject().put("params", params).put("user", siteRequest.getUserPrincipal());
					JsonObject json = new JsonObject().put("context", context);
					eventBus.request(CompanyCourse.getClassApiAddress(), json, new DeliveryOptions().addHeader("action", "patchCompanyCourseFuture")).onSuccess(c -> {
			...
			LOG.error(String.format("refreshCompanyCourse failed. "), ex);
			promise.fail(ex);
		}
		return promise.future();
	}
