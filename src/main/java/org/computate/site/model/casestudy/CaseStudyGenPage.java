 ...
 * Generated: true
 **/
public class CaseStudyGenPage extends CaseStudyGenPageGen<BaseResultPage> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListCaseStudy_(Wrap<SearchList<CaseStudy>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListCaseStudy_ != null)
      w.o(JsonObject.mapFrom(searchListCaseStudy_.getResponse()).toString());
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(searchListCaseStudy_.getResponse().getStats());
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(searchListCaseStudy_.getResponse().getFacetCounts());
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListCaseStudy_.getStart().longValue();
    Long rows = searchListCaseStudy_.getRows().longValue();
    Long foundNum = searchListCaseStudy_.getResponse().getResponse().getNumFound().longValue();
  ...
  protected void _varsQ(JsonObject vars) {
    CaseStudy.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(CaseStudy.displayNameCaseStudy(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CaseStudy.classSimpleNameCaseStudy(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListCaseStudy_.getRequest().getQuery()).filter(fq -> fq.startsWith(CaseStudy.varIndexedCaseStudy(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    CaseStudy.varsFqForClass().forEach(var -> {
      String varIndexed = CaseStudy.varIndexedCaseStudy(var);
      String varStored = CaseStudy.varStoredCaseStudy(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(CaseStudy.displayNameCaseStudy(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(CaseStudy.classSimpleNameCaseStudy(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListCaseStudy_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(CaseStudy.varIndexedCaseStudy(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
  ...
  protected void _varsRange(JsonObject vars) {
    CaseStudy.varsRangeForClass().forEach(var -> {
      String varIndexed = CaseStudy.varIndexedCaseStudy(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(CaseStudy.displayNameCaseStudy(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = searchListCaseStudy_.getResponse().getResponse().getNumFound().longValue();
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListCaseStudy_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListCaseStudy_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListCaseStudy_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = CaseStudy.searchVarCaseStudy(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", CaseStudy.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListCaseStudy_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = CaseStudy.searchVarCaseStudy(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", CaseStudy.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListCaseStudy_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListCaseStudy_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListCaseStudy_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListCaseStudy_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return CaseStudy.searchVarCaseStudy(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListCaseStudy_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCaseStudy_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCaseStudy_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListCaseStudy_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListCaseStudy_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = CaseStudy.searchVarCaseStudy(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListCaseStudy_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListCaseStudy_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listCaseStudy(JsonArray l) {
    Optional.ofNullable(searchListCaseStudy_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _caseStudyCount(Wrap<Integer> w) {
    w.o(searchListCaseStudy_ == null ? 0 : searchListCaseStudy_.size());
  }

  protected void _caseStudy_(Wrap<CaseStudy> w) {
    if(caseStudyCount == 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("id")).orElse(null) != null)
      w.o(searchListCaseStudy_.get(0));
  }

  protected void _id(Wrap<String> w) {
    if(caseStudy_ != null)
      w.o(caseStudy_.getId());
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(caseStudy_ != null && caseStudy_.getObjectTitle() != null)
      c.o(caseStudy_.getObjectTitle());
    else if(caseStudy_ != null)
      c.o("case studies");
    else if(searchListCaseStudy_ == null || caseStudyCount == 0)
      c.o("no case study found");
    else
      c.o("case studies");
  }
