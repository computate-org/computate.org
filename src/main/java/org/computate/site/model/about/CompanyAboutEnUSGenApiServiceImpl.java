...
public class CompanyAboutEnUSGenApiServiceImpl extends BaseApiServiceImpl implements CompanyAboutEnUSGenApiService {
	...
	public void patchCompanyAbout(JsonObject body, ServiceRequest serviceRequest, Handler<AsyncResult<ServiceResponse>> eventHandler) {
		...
		user(serviceRequest, SiteRequest.class, SiteUser.class, SiteUser.getClassApiAddress(), "postSiteUserFuture", "patchSiteUserFuture", classPublicRead).onSuccess(siteRequest -> {
			...
			form.add("permission", String.format("%s#%s", CompanyAbout.CLASS_AUTH_RESOURCE, config.getString(ComputateConfigKeys.AUTH_SCOPE_ADMIN)));
			form.add("permission", String.format("%s#%s", CompanyAbout.CLASS_AUTH_RESOURCE, config.getString(ComputateConfigKeys.AUTH_SCOPE_SUPER_ADMIN)));
			...
			form.add("permission", String.format("%s#%s", CompanyAbout.CLASS_AUTH_RESOURCE, "PUT"));
			if(pageId != null)
	...
	public void searchpageCompanyAboutPageInit(CompanyAboutPage page, SearchList<CompanyAbout> listCompanyAbout, Promise<Void> promise) {
		promise.complete();
	}
		...
		try {
			...
			page.promiseDeepCompanyAboutPage(siteRequest).onSuccess(a -> {
				try {
					...
					searchpageCompanyAboutPageInit(page, listCompanyAbout, promise1);
	...
	public void editpageCompanyAboutPageInit(CompanyAboutPage page, SearchList<CompanyAbout> listCompanyAbout, Promise<Void> promise) {
		promise.complete();
	}
		...
		try {
			...
			page.promiseDeepCompanyAboutPage(siteRequest).onSuccess(a -> {
				try {
					...
					ctx.mergeIn(JsonObject.mapFrom(page));
					Promise<Void> promise1 = Promise.promise();
					editpageCompanyAboutPageInit(page, listCompanyAbout, promise1);
					promise1.future().onSuccess(b -> {
						String renderedTemplate = jinjava.render(template, ctx.getMap());
						Buffer buffer = Buffer.buffer(renderedTemplate);
						promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
	...
	public void displaypageCompanyAboutPageInit(CompanyAboutPage page, SearchList<CompanyAbout> listCompanyAbout, Promise<Void> promise) {
		promise.complete();
	}
		...
		try {
			...
			page.promiseDeepCompanyAboutPage(siteRequest).onSuccess(a -> {
				try {
					...
					ctx.mergeIn(JsonObject.mapFrom(page));
					Promise<Void> promise1 = Promise.promise();
					displaypageCompanyAboutPageInit(page, listCompanyAbout, promise1);
					promise1.future().onSuccess(b -> {
						String renderedTemplate = jinjava.render(template, ctx.getMap());
						Buffer buffer = Buffer.buffer(renderedTemplate);
						promise.complete(new ServiceResponse(200, "OK", buffer, requestHeaders));
