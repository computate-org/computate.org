 ...
 * Generated: true
 **/
public class SwitchToLinuxGenPage extends SwitchToLinuxGenPageGen<PageLayout> {

  /**
   * {@inheritDoc}
   * Ignore: true
   **/
  protected void _searchListSwitchToLinux_(Wrap<SearchList<SwitchToLinux>> w) {
  }

  @Override
  protected void _pageResponse(Wrap<String> w) {
    if(searchListSwitchToLinux_ != null)
      w.o(Optional.ofNullable(searchListSwitchToLinux_.getResponse()).map(response -> JsonObject.mapFrom(response).toString()).orElse(null));
  }

  @Override
  protected void _stats(Wrap<SolrResponse.Stats> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getResponse()).map(response -> response.getStats()).orElse(null));
  }

  @Override
  protected void _facetCounts(Wrap<SolrResponse.FacetCounts> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getResponse()).map(response -> response.getFacetCounts()).orElse(null));
  }

  @Override
  protected void _pagination(JsonObject pagination) {
    JsonArray pages = new JsonArray();
    Long start = searchListSwitchToLinux_.getStart().longValue();
    Long rows = searchListSwitchToLinux_.getRows().longValue();
    Long foundNum = Optional.ofNullable(searchListSwitchToLinux_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListSwitchToLinux_.getList().size()));
  ...
  protected void _varsQ(JsonObject vars) {
    SwitchToLinux.varsQForClass().forEach(var -> {
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(SwitchToLinux.displayNameSwitchToLinux(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SwitchToLinux.classSimpleNameSwitchToLinux(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", Optional.ofNullable(searchListSwitchToLinux_.getRequest().getQuery()).filter(fq -> fq.startsWith(SwitchToLinux.varIndexedSwitchToLinux(var) + ":")).map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _varsFq(JsonObject vars) {
    ...
    varsFqCount = 0;
    for(String var : SwitchToLinux.varsFqForClass()) {
      String varIndexed = SwitchToLinux.varIndexedSwitchToLinux(var);
      String varStored = SwitchToLinux.varStoredSwitchToLinux(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      ...
      json.put("displayName", Optional.ofNullable(SwitchToLinux.displayNameSwitchToLinux(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SwitchToLinux.classSimpleNameSwitchToLinux(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      Object v = searchListSwitchToLinux_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SwitchToLinux.varIndexedSwitchToLinux(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null);
      if(v != null) {
        Matcher mFq = Pattern.compile("(\\w+):(.+?(?=(\\)|\\s+OR\\s+|\\s+AND\\s+|$)))").matcher(SearchTool.unescapeQueryChars((String)v));
        StringBuffer sb = new StringBuffer();
        while(mFq.find()) {
          String entityVar = SwitchToLinux.searchVarSwitchToLinux(varIndexed);
  ...
  protected void _varsRange(JsonObject vars) {
    SwitchToLinux.varsRangeForClass().forEach(var -> {
      String varIndexed = SwitchToLinux.varIndexedSwitchToLinux(var);
      JsonObject json = new JsonObject();
      json.put("var", var);
      json.put("displayName", Optional.ofNullable(SwitchToLinux.displayNameSwitchToLinux(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("classSimpleName", Optional.ofNullable(SwitchToLinux.classSimpleNameSwitchToLinux(var)).map(d -> StringUtils.isBlank(d) ? var : d).orElse(var));
      json.put("val", searchListSwitchToLinux_.getRequest().getFilterQueries().stream().filter(fq -> fq.startsWith(SwitchToLinux.varIndexedSwitchToLinux(var) + ":")).findFirst().map(s -> SearchTool.unescapeQueryChars(StringUtils.substringAfter(s, ":"))).orElse(null));
      vars.put(var, json);
    });
  }
  ...
  protected void _query(JsonObject query) {
    ...
    Long num = Optional.ofNullable(searchListSwitchToLinux_.getResponse()).map(response -> response.getResponse().getNumFound().longValue()).orElse(Long.valueOf(searchListSwitchToLinux_.getList().size()));
    String q = "*:*";
    ...
    Long rows1 = Optional.ofNullable(searchListSwitchToLinux_).map(l -> l.getRows()).orElse(10L);
    Long start1 = Optional.ofNullable(searchListSwitchToLinux_).map(l -> l.getStart()).orElse(1L);
    ...
    for(String fq : Optional.ofNullable(searchListSwitchToLinux_).map(l -> l.getFilterQueries()).orElse(Arrays.asList())) {
      if(!StringUtils.contains(fq, "(")) {
        String fq1 = SwitchToLinux.searchVarSwitchToLinux(StringUtils.substringBefore(fq, ":"));
        String fq2 = StringUtils.substringAfter(fq, ":");
        if(!StringUtils.startsWithAny(fq, "classCanonicalNames_", "archived_", "sessionId", "userKeys"))
          fqs.put(fq1, new JsonObject().put("var", fq1).put("val", fq2).put("displayName", SwitchToLinux.displayNameForClass(fq1)));
        }
      }
    query.put("fq", fqs);

    JsonArray sorts = new JsonArray();
    for(String sort : Optional.ofNullable(searchListSwitchToLinux_).map(l -> l.getSorts()).orElse(Arrays.asList())) {
      String sort1 = SwitchToLinux.searchVarSwitchToLinux(StringUtils.substringBefore(sort, " "));
      sorts.add(new JsonObject().put("var", sort1).put("order", StringUtils.substringAfter(sort, " ")).put("displayName", SwitchToLinux.displayNameForClass(sort1)));
    }
  ...
  protected void _rangeGap(Wrap<String> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.gap", null) != null)
      w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetRangeGap()).orElse(null));
  }
  ...
  protected void _rangeEnd(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.end", null) != null)
      w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetRangeEnd()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _rangeStart(Wrap<ZonedDateTime> w) {
    if(serviceRequest.getParams().getJsonObject("query").getString("facet.range.start", null) != null)
      w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetRangeStart()).map(s -> TimeTool.parseZonedDateTime(defaultTimeZone, s)).orElse(null));
  }
  ...
  protected void _defaultRangeVar(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetRanges()).orElse(Optional.ofNullable(defaultRangeStats).map(s -> Arrays.asList(s.getString("defaultRangeVar"))).orElse(Arrays.asList())).stream().findFirst().map(v -> { if(v.contains("}")) return StringUtils.substringBefore(StringUtils.substringAfterLast(v, "}"), "_"); else return SwitchToLinux.searchVarSwitchToLinux(v); }).orElse("created"));
  }

  @Override
  protected void _defaultFacetSort(Wrap<String> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetSort()).orElse("index"));
  }

  @Override
  protected void _defaultFacetLimit(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetLimit()).orElse(1));
  }

  @Override
  protected void _defaultFacetMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetMinCount()).orElse(1));
  }

  @Override
  protected void _defaultPivotMinCount(Wrap<Integer> w) {
    w.o(Optional.ofNullable(searchListSwitchToLinux_.getFacetPivotMinCount()).orElse(0));
  }
  ...
  protected void _defaultSortVars(List<String> l) {
    if(!searchListSwitchToLinux_.getDefaultSort()) {
      Optional.ofNullable(searchListSwitchToLinux_.getSorts()).orElse(Arrays.asList()).forEach(varSortStr -> {
        String varSortParts[] = varSortStr.split(" ");
        String varSort = SwitchToLinux.searchVarSwitchToLinux(varSortParts[0]);
  ...
  protected void _defaultFieldListVars(List<String> l) {
    Optional.ofNullable(searchListSwitchToLinux_.getFields()).orElse(Arrays.asList()).forEach(varStored -> {
      ...
      for(String part : parts) {
        if(StringUtils.isNotBlank(part)) {
          String var = SwitchToLinux.searchVarSwitchToLinux(part);
  ...
  protected void _defaultStatsVars(List<String> l) {
    Optional.ofNullable(searchListSwitchToLinux_.getStatsFields()).orElse(Arrays.asList()).forEach(varIndexed -> {
  ...
  protected void _defaultPivotVars(List<String> l) {
    Optional.ofNullable(searchListSwitchToLinux_.getFacetPivots()).orElse(Arrays.asList()).forEach(facetPivot -> {
   ...
   * {@inheritDoc}
   **/
  protected void _listSwitchToLinux(JsonArray l) {
    Optional.ofNullable(searchListSwitchToLinux_).map(o -> o.getList()).orElse(Arrays.asList()).stream().map(o -> JsonObject.mapFrom(o)).forEach(o -> l.add(o));
  }

  protected void _resultCount(Wrap<Integer> w) {
    w.o(searchListSwitchToLinux_ == null ? 0 : searchListSwitchToLinux_.size());
  }

  /**
   * Initialized: false
  **/
  protected void _result(Wrap<SwitchToLinux> w) {
    if(resultCount >= 1 && Optional.ofNullable(siteRequest_.getServiceRequest().getParams().getJsonObject("path")).map(o -> o.getString("pageId")).orElse(null) != null)
      w.o(searchListSwitchToLinux_.get(0));
  }
  ...
  protected void _pageTitle(Wrap<String> c) {
    if(result != null && result.getObjectTitle() != null)
      ...
      c.o("Switch to Linux");
    else if(searchListSwitchToLinux_ == null || resultCount == 0)
      c.o("no Switch to Linux found");
    else
      c.o("Switch to Linux");
  }
  ...
  protected void _classAllName(Wrap<String> w) {
    w.o("all Switch to Linux");
  }

  @Override
  protected void _pageUri(Wrap<String> w) {
    if("enUS".equals(lang))
      w.o("/en-us/search/switch-to-linux");
  }

  @Override
  protected void _apiUri(Wrap<String> w) {
    w.o("/en-us/api/switch-to-linux");
  }
  ...
  protected void _pageDescription(Wrap<String> c) {
      c.o("Learn how to switch to Linux to run and develop open source cloud applications in OpenShift Local on your own computer. ");
  }

  @Override
  protected void _pageImageUri(Wrap<String> c) {
      c.o("/png/en-us/search/switch-to-linux-999.png");
  }

  @Override
  protected void _classIcon(Wrap<String> c) {
      c.o("<i class=\"fa-brands fa-linux\"></i>");
  }

  protected void _pageUriSwitchToLinux(Wrap<String> c) {
      c.o("/en-us/search/switch-to-linux");
  }
}
